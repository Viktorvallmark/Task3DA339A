...
import org.example.U3.view.CustomPizzaFrame;
...
public class Controller {
    ...
    public void addItemToOrder(int selectionIndex) {
        ...
        if (selectionIndex != -1) { // if something is selected in the left menu list
            switch (currentLeftMenu) { // This might need to change depending on architecture
                case Cake:
                    currentOrderArray[nbrOfOrders] = cakeMenuString[selectionIndex]
                            .toString(); // for test purposes - needs to be replaced with solution of finding
                    // chosen menu item matching architecture for model
                    break;
                case PerUnitItem:
                    currentOrderArray[nbrOfOrders] = perUnitItemMenuString[selectionIndex].toString(); // see comment
                    // for case above
                    break;
            }
            nbrOfOrders++; // for test purposes - need to be removed or changed when model for handling
            // orders is implemented
            costCurrentOrder = costCurrentOrder
                    + 100; // for test purposes - replace with calculation of cost when how orders are
            // handled is implemented in model
    ...
    public void viewSelectedOrder(int selectionIndex) {
        ...
        if ((selectionIndex != -1) && currentLeftMenu == ButtonType.OrderHistory) {
            costCurrentOrder = 100; // for test purposes - replace with calculation of cost when how orders are
            // handled
            // is implemented in model
            view.populateRightPanel(
                    order1Simulation); // update left panel with order details - this takes a shortcut in
    ...
    public void placeOrder() {
        System.out.println(
                "Pressed Order to create a new order"); // for test purposes - remove when not needed more
